...
Pour examiner la question relative à la corrélation entre le vent et la diffusion des incendies sur le sol français, nous adopterons une approche distincte en segmentant ce sujet en cinq sous-questions.

C'est pourquoi, pour aborder ce problème, on devrait considérer ces cinq enjeux :

1.  Force du vent
2.  Surface parcourue par le feu
3.  Surface parcourue par le feu en fonction de la Force du vent
4.  Force du vent par zone géographique
5.  Surface parcourue par le feu par zone géographique
...
Dans cette sous-problmatique on va analyser la surface parcourure par le feu pour en pouvoir arrive a notre but et a anlayser la relation entre le vent et la propagation des incendies donc pour ce qui concerne la propagation ca ve dire en d'autres terme c'est la surface parcourue par le feu c'est ce qu'on va analyser maintenant dans cette sous-problematique 

```{r}
library(ggplot2)
data <- read.csv("../Exports/export_vents.csv")
ggplot(data, aes(x = surface_parcourue_m2)) +
  geom_histogram(binwidth = 1, fill = "violet", color = "black", alpha = 0.7) +
  labs(title = "Histogramme de la Surface Parcourut par les Incendies", x = "Surface Parcourue", y = "Fréquence") +
  theme_minimal()
```


###### 4.3.2.3.2 Surface parcourue par le feu en fonction de la Force du vent
###### 4.3.2.3.3 Force du vent par zone géographique
###### 4.3.2.3.4 Surface parcourue par le feu par zone géographique
